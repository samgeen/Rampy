program star2list
  ...
  !================================================
  real(KIND=8),dimension(:),allocatable::m,age,zsol
  !================================================
...
!  read(10,'("H0          =",E23.15)')h0
  ...
  write(12,*)'     Id   Mass(Msol)      x(Lbox)      y(Lbox)      z(Lbox)     Age(Myr)'       
  do k=1,ncpu_read
     icpu=cpu_list(k)
     call title(icpu,ncharcpu)
     nomfich=TRIM(repository)//'/part_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
     ...
     do i=1,npart2
        ...
        if(ok_part)then
           ...
           write(12,'(I8,1X,10(1PE12.5,1X))')id(i),m(i)*unit_d*(unit_l**3)/2d33,x(i,1),x(i,2),x(i,3),time*1000.,zsol(i)/0.02
        end if
     end do
     deallocate(x,v,m)
