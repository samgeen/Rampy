...
subroutine cmpdt(uu,gg,dx,dt,ncell)
  ...
  use const
  use rt_cooling_module,only:rt_isoPress,iRTisoPressVar                     !RTPress
  use rt_parameters,only:rt_advect                                          !RTpress
  implicit none
  integer::ncell
  real(dp)::dx,dt
  ...
  ! Isotropic pressure from isotropic radiation:
  if(rt_isoPress .and. rt_advect) then                                !RTpress
     do k=1,ncell                                                     !RTpress
        uu(k,ndim+2) = uu(k,ndim+2) + uu(k,iRTisoPressVar) / uu(k,1)  !RTpress
     end do                                                           !RTpress
  endif                                                               !RTpress
...
subroutine riemann_hllc(qleft,qright,qgdnv,fgdnv,ngrid)
  ...
  do i=1,ngrid
     ...
     ! Here update Ptot but not etot, according to O. Agertz !joki!!!!      !RTpress
     ! Add isotropic radiation pressure                                     !RTpress
     if(rt_isoPress .and. rt_advect) then                                   !RTpress
        Pl = Pl + MAX(qleft(i,iRTisoPressVar),0d0)                          !RTpress
     endif                                                                  !RTpress
     Ptotl = Pl
     ...
     ! Add isotropic radiation pressure                                     !RTpress
     if(rt_isoPress .and. rt_advect) then                                   !RTpress
        Pr = Pr + MAX(qright(i,iRTisoPressVar),0d0)                         !RTpress
     endif                                                                  !RTpress
     Ptotr = Pr
